...
class EntraIDPEASS():
    ...
    def get_my_user_id(self):
        ...
        resp = requests.get("https://graph.microsoft.us/v1.0/me?$select=id", headers=self.headers)
    ...
    def get_role_name(self, role_definition_id):
        url = f"https://graph.microsoft.us/v1.0/roleManagement/directory/roleDefinitions/{role_definition_id}"
    ...
    def get_granular_permissions(self, role_id):
        ...
        url = f"https://graph.microsoft.us/v1.0/roleManagement/directory/roleDefinitions/{role_id}"
        resp = requests.get(url, headers=self.headers)
    ...
    def get_entraid_memberships(self):
        sub_resources = []
        sub_resources_tmp = []
        memberOf_url = "https://graph.microsoft.us/v1.0/me/transitiveMemberOf"
        ...
        url = f"https://graph.microsoft.us/v1.0/roleManagement/directory/roleAssignmentScheduleInstances?$filter=principalId eq '{user_id}'&$expand=roleDefinition"
    ...
    def get_my_app_role_assignments(self):
        ...
        url = "https://graph.microsoft.us/v1.0/me/appRoleAssignments"
        assignments = self.get_all_pages(url)
    ...
    def _get_app_role_value(self, resource_id, app_role_id):
        ...
        """
        url = f"https://graph.microsoft.us/v1.0/servicePrincipals/{resource_id}"
        resp = requests.get(url, headers=self.headers)
        if resp.status_code != 200:
    ...
    def get_eligible_roles(self):
        user_id = self.get_my_user_id()
        url = f"https://graph.microsoft.us/v1.0/roleManagement/directory/roleEligibilityScheduleInstances?$filter=principalId eq '{user_id}'&$expand=roleDefinition"
    ...
    def get_api_permissions(self):
        ...
        app_url = f"https://graph.microsoft.us/v1.0/applications?$filter=appId eq '{app_id}'&$select=id"
        ...
        req_access_url = f"https://graph.microsoft.us/v1.0/applications/{app_object_id}?$select=requiredResourceAccess"
    ...
    def get_entraid_owns(self):
        ...
        owned_objects_url = "https://graph.microsoft.us/v1.0/me/ownedObjects?$select=id,displayName,appDisplayName"
